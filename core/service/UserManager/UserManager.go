...
func (root *Manager) AddUser(userInfo OAuth.UserInfo) error {
	//检查用户是否存在
	if row := root.DB.Where("id = ?", md5.GenMD5(userInfo.ID)).Find(&models.Users{}).RowsAffected; row > 0 {
	...
	//创建用户
	err := root.DB.Create(&models.Users{
		Name: userInfo.Name,
...
func (root *Manager) DeleteUserByID(userID string) error {
	//检查用户是否存在
	if row := root.DB.Where("id = ?", userID).Find(&models.Users{}).RowsAffected; row == 0 {
	...
	//获取用户名下文章
	var articleID []uint
	err := root.DB.Model(&models.Article{}).Where("author_id = ?", userID).Pluck("article_id", &articleID).Error
	if err != nil {
		i18n.GetI18n(root.c).ServerError(err)
		return err
	}
	...
	err = root.DB.Model(&models.Comment{}).Where("article_id IN (?)", articleID).Pluck("id", &ArticleCommentID).Error
	if err != nil {
		i18n.GetI18n(root.c).ServerError(err)
		return err
	}
	...
	//删除用户名下的文章
	if len(articleID) != 0 {
		err = root.DB.Where("author_id = ?", userID).Delete(&models.Article{}).Error
		if err != nil {
			i18n.GetI18n(root.c).ServerError(err)
			return err
		}
	...
	if len(ArticleCommentID) != 0 {
		err = root.DB.Where("id IN (?)", ArticleCommentID).Delete(&models.Comment{}).Error
		if err != nil {
			i18n.GetI18n(root.c).ServerError(err)
			return err
		}
	...
	err = root.DB.Model(&models.Comment{}).Where("author_id = ?", userID).Pluck("id", &userCommentsID).Error
	if err != nil {
		i18n.GetI18n(root.c).ServerError(err)
		return err
	}
	...
	for _, id := range userCommentsID {
		err = DeleteCommentAndChildren(root.DB, id)
		if err != nil {
			i18n.GetI18n(root.c).ServerError(err)
			return err
		}
	...
	//删除用户
	err = root.DB.Where("id = ?", userID).Unscoped().Delete(&models.Users{}).Error
	if err != nil {
		i18n.GetI18n(root.c).ServerError(err)
		return err
	}
...
func (root *Manager) GetUserInfo(sso string) error {
	...
	err := root.DB.Where("id = ?", sso).First(&userInfo).Error
