...
func (root *Manager) AddComment() error {
	...
	var uname []string
	err = root.db.Model(&models.Users{}).Where("id = ?", AuthorID).Pluck("name", &uname).Error
	if err != nil {
		i18n.GetI18n(root.c).ServerError(err)
		return err
	}
...
func (root *Manager) AddSubComment() error {
	...
	//获取父评论
	father := root.db.Model(&models.Comment{}).Where("id = ?", childComment.ParentID)

	var fatherLevel uint

	err = father.Pluck("level", &fatherLevel).Error
...
func (root *Manager) GetComment() error {
	...
	err := root.db.Where("article_id = ?", ArticleID).Order("created_at DESC").Find(&AllComments).Error
