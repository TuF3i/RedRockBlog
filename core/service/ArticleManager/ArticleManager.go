...
func (root *ArticleManager) AddWork() error {
	...
	err = root.db.Create(&NewArticle).Error
	if err != nil {
		i18n.GetI18n(root.c).ServerError(err)
		return err
	}
...
func (root *ArticleManager) DeleteWork() error {
	...
	err = root.db.Where("author_id = ? AND article_id = ?", AuthorID, dataConv.InitConv().Str2uint(ArticleID)).Delete(&models.Article{}).Error
	...
	err = root.db.Where("article_id = ?", dataConv.InitConv().Str2uint(ArticleID)).Delete(&models.Comment{}).Error
...
func (root *ArticleManager) GetMyWorkList() error {
	...
	myWork := root.db.Model(&models.Article{}).
...
func (root *ArticleManager) GetGlobalArticleList() error {
	...
	Work := root.db.Model(&models.Article{}).
...
func (root *ArticleManager) SearchArticleByExtTitle() error {
	...
	err := root.db.Model(&models.Article{}).Where("ext_title = ?", ArticleExtName).Select(
		"language_id, " +
			"article_id, " +
			"title, " +
			"ext_title, " +
...
func (root *ArticleManager) UpdateWork() error {
	...
	err = root.db.Where("article_id = ?", Article.ArticleID).First(&checker).Error
	...
	err = root.db.Model(&models.Article{}).Where("article_id = ?", Article.ArticleID).Omit("created_at").Updates(&NewArticle).Error
	if err != nil {
		i18n.GetI18n(root.c).ServerError(err)
		return err
	}
...
func (root *ArticleManager) GetWorkDetail() error {
	...
	err = root.db.Where("author_id = ? AND article_id = ?",
...
func (root *ArticleManager) GetArticle() error {
	...
	if session != "" {
		...
		err := root.db.Where("article_id = ?", dataConv.InitConv().Str2uint(ArticleID)).First(&Article).Error
